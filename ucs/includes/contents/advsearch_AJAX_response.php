...
if ($_POST['type'] == 'author') {
    $data_q = $dbs->query("SELECT author_name FROM mst_author WHERE author_name LIKE '%$searchVal%' LIMIT 5");
} else {
    $data_q = $dbs->query("SELECT topic FROM mst_topic WHERE topic LIKE '%$searchVal%' LIMIT 5");
}
