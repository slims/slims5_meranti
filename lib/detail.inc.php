...
class detail extends content_list
{
    ...
    protected function htmlOutput()
    {
        ...
        while ($data = $_biblio_authors_q->fetch_row()) {
            $authors .= '<a href="?author='.urlencode('"'.$data[0].'"').'&search=Search" title="'._('Click to view others documents with this author').'">'.$data[0]."</a><br />";
        ...
        while ($data = $_biblio_topics_q->fetch_row()) {
            $topics .= '<a href="?subject='.urlencode('"'.$data[0].'"').'&search=Search" title="'._('Click to view others documents with this subject').'">'.$data[0]."</a><br />";
            $this->metadata .= $data[0].'; ';
        }
        $this->metadata .= '" />';
        ...
        if ($_item_loc_q->num_rows < 1) {
            $this->record_detail['location'] = '<strong style="color: red; font-weight: bold;">'._('There is no item/copy for this title yet').'</strong>';
        } else {
            $this->record_detail['location'] = '';
            while ($_item_loc_d = $_item_loc_q->fetch_row()) {
                $this->record_detail['location'] .= '<strong>'.$_item_loc_d[1].' </strong> '._('copies at').' <strong>'.$_item_loc_d[0].'</strong><br />'; //mfc
            }
        }
