...
if (isset($_GET['search']) AND !empty($_GET['search'])) {
    ...
    if ($is_adv) {
        $info .= '<div style="clear: both;">'._('Found  <strong>{biblio_list->num_rows}</strong> from your keywords').' : </div>  '; //mfc
        ...
        $info .= '<div style="clear: both;">'._('Found  <strong>{biblio_list->num_rows}</strong> from your keywords').': <strong><cite>'.$keywords.'</cite></strong></div>'; //mfc
    }
} else {
...
if (isset($_GET['page']) AND $_GET['page'] > 1) {
    ...
    $msg = str_replace('{page}', $page, _('You currently on page <strong>{page}</strong> of <strong>{total_pages}</strong> page(s)')); //mfc
...
// query time
$info .= '<div>'._('Query took').' <b>'.$biblio_list->query_time.'</b> '._('second(s) to complete').'</div>'; //mfc
