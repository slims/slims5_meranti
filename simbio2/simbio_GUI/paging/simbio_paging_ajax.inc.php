...
class simbio_paging_ajax
{
     ...
     * @return  string
     */
    public static function paging($int_all_recs_num, $int_recs_each_page, $int_pages_each_set = 10, $str_fragment = '')
    {
        ...
        if ($_page > 1) {
            $_buffer .= ' &nbsp;';
            $_buffer .= '<a href="javascript: setContent(\'mainContent\', \''.$_current_page.(1).$str_fragment.'\', \'post\');">'.$_first.'</a>&nbsp; '."\n";
            $_buffer .= ' &nbsp;';
            $_buffer .= '<a href="javascript: setContent(\'mainContent\', \''.$_current_page.($_page-1).$str_fragment.'\', \'post\');">'.$_prev.'</a>&nbsp; '."\n";
        }
        ...
        for ($p = $_pager_offset; ($p <= $_num_page_total) AND ($_stopper < $int_pages_each_set+1); $p++) {
            if ($p == $_page) {
                ...
                $_buffer .= ' &nbsp;';
                $_buffer .= '<a href="javascript: setContent(\'mainContent\', \''.$_current_page.$p.$str_fragment.'\', \'post\');">'.$p.'</a>&nbsp; '."\n";
            }
        ...
        if (($_pager_offset != $_num_page_total-4) AND ($_page != $_num_page_total)) {
            $_buffer .= ' &nbsp;';
            $_buffer .= '<a href="javascript: setContent(\'mainContent\', \''.$_current_page.($_page+1).$str_fragment.'\', \'post\');">'.$_next.'</a>&nbsp; '."\n";
        }
        ...
        if ($_page < $_num_page_total) {
            $_buffer .= ' &nbsp;';
            $_buffer .= '<a href="javascript: setContent(\'mainContent\', \''.$_current_page.($_num_page_total).$str_fragment.'\', \'post\');">'.$_last.'</a>&nbsp; '."\n";
        }
