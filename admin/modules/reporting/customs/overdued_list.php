...
?>
    ...
    <legend style="font-weight: bold"><?php echo strtoupper(lang_mod_circ_overdues); ?> - <?php echo lang_mod_reporting_form_generic_header; ?></legend>
...
<?php
    ...
    function showOverduedList($obj_db, $array_data)
    {
        ...
        while ($ovd_title_d = $ovd_title_q->fetch_assoc()) {
            ...
            $_buffer .= '<td valign="top" width="40%">'.$ovd_title_d['title'].'<div>'.lang_mod_biblio_item_field_price.': '.$ovd_title_d['price'].' '.$ovd_title_d['price_currency'].'</div></td>';
            $_buffer .= '<td width="20%">'.lang_mod_report_overdues_table_overdue.': '.$ovd_title_d['Overdue Days'].' '.lang_mod_report_overdues_table_overdue_days.'</td>';
