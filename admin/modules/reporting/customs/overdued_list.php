...
?>
    ...
    <legend style="font-weight: bold"><?php echo strtoupper(_('Overdued List')); ?> - <?php echo _('Report Filter'); ?></legend>
...
<?php
    ...
    function showOverduedList($obj_db, $array_data)
    {
        ...
        $_buffer .= '<div style="font-size: 10pt; margin-bottom: 3px;">'._('E-mail').': <a href="mailto:'.$member_d[1].'">'.$member_d[1].'</a> - '._('Phone Number').': '.$member_d[2].'</div>';
        ...
        while ($ovd_title_d = $ovd_title_q->fetch_assoc()) {
            ...
            $_buffer .= '<td valign="top" width="40%">'.$ovd_title_d['title'].'<div>'._('Price').': '.$ovd_title_d['price'].' '.$ovd_title_d['price_currency'].'</div></td>';
            $_buffer .= '<td width="20%">'._('Overdue').': '.$ovd_title_d['Overdue Days'].' '._('day(s)').'</td>';
            $_buffer .= '<td width="30%">'._('Loan Date').': '.$ovd_title_d['loan_date'].' &nbsp; '._('Due Date').': '.$ovd_title_d['due_date'].'</td>';
            $_buffer .= '</tr>';
        }
        $_buffer .= '</table>';
        return $_buffer;
