...
$loan_report[_('Total Loan')] = $report_data[0];
...
$loan_report[_('Total Loan By GMD/Medium')] = $report_data;
...
$loan_report[_('Total Loan By Collection Type')] = $report_data;
...
$loan_report[_('Total Loan Transactions')] = $report_data;
...
$loan_report[_('Transaction Average (Per Day)')] = @ceil($loan_report[_('Total Loan Transactions')]/$total_loan_days);

// peak transaction
$loan_report[_('Total Peak Transaction')] = $peak_transaction_data[0];

// total members having loans
$report_query = $dbs->query('SELECT DISTINCT member_id FROM loan');
$report_data = $report_query->num_rows;
$loan_report[_('Members Already Had Loans')] = $report_data;
...
$loan_report[_('Members Never Have Loans Yet')] = $total_members_data[0]-$loan_report[_('Members Already Had Loans')];
...
$report_data = $report_query->fetch_row();
$loan_report[_('Total Overdued Loans')] = $report_data[0];

// table header
$table->setHeader(array(_('Loan Data Summary')));
...
if (isset($_GET['print'])) {
    ...
    $html_str .= '</head>';
    $html_str .= '<body>'."\n";
    $html_str .= '<h3>'.$sysconf['library_name'].' - '._('Loan Report').'</h3>';
    $html_str .= '<hr size="1" />';
...
<div class="menuBoxInner statisticIcon">
    <?php echo strtoupper(_('Loan Report')); ?>
    <hr />
