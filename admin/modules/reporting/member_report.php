...
$loan_report[_('Total Registered Members')] = $report_data[0];

// total number of active member
$report_query = $dbs->query('SELECT COUNT(member_id) FROM member
    WHERE TO_DAYS(expire_date)>TO_DAYS(\''.date('Y-m-d').'\')');
$report_data = $report_query->fetch_row();
$loan_report[_('Total Active Member')] = $report_data[0];
...
$loan_report[_('Total Members By Membership Type')] = $report_data;

// total expired member
$report_query = $dbs->query('SELECT COUNT(member_id) FROM member
    WHERE TO_DAYS(\''.date('Y-m-d').'\')>TO_DAYS(expire_date)');
$report_data = $report_query->fetch_row();
$loan_report[_('Total Expired Member')] = $report_data[0];
...
$loan_report[_('10 most active members')] = $report_data;

// table header
$table->setHeader(array(_('Membership Data Summary')));
...
if (isset($_GET['print'])) {
    ...
    $html_str .= '</head>';
    $html_str .= '<body>'."\n";
    $html_str .= '<h3>'.$sysconf['library_name'].' - '._('Membership Report').'</h3>';
    $html_str .= '<hr size="1" />'."\n";
...
<div class="menuBoxInner statisticIcon">
    <?php echo strtoupper(_('Membership Report')); ?>
    <hr />
