...
if ($stk_q->num_rows) {
    echo '<div class="errorBox">'._('There is already stock taking proccess running!').'</div>';
} else {
    // add new stock take
    if (isset($_POST['saveData']) AND empty($_POST['name'])) {
        utility::jsAlert(_('Stock Take Name must be filled!'));
        exit();
        ...
        if ($sql_op->insert('stock_take', $data)) {
            ...
            if ($total_rows_d[0] > 0) {
                ...
                utility::jsAlert(_('Stock Taking Initialized'));
                ...
                utility::jsAlert(_('Stock Taking FAILED to Initialized.\nNo item to stock take!'));
            }
            exit();
        }
    }
    ...
    $form->submit_button_attr = 'name="saveData" value="'._('Initialize Stock Take').'" class="button"';
    ...
    /* Form Element(s) */
    // stock take name
    $form->addTextField('text', 'name', _('Stock Take Name').'*', '', 'style="width: 60%;"');
    // gmd
    ...
    $form->addSelectList('gmdID', _('GMD'), $gmd_options);
    // collection type
    ...
    $form->addSelectList('collTypeID', _('Collection Type'), $coll_type_options);
    // location
        ...
        $location_options[] = array('0', _('ALL'));
        while ($location_d = $location_q->fetch_row()) {
            $location_options[] = array($location_d[0], $location_d[1]);
        }
    $form->addSelectList('location', _('Location'), $location_options);
    // item site
    $form->addTextField('text', 'itemSite', _('Shelf Location'), '', 'style="width: 20%;"');
    // classification;
    $str_input = simbio_form_element::textField('text', 'classification', '', 'style="width: 60%;"');
    $str_input .= '<br />'._('Separate each class comma sign. Use * for wildcard');
    $form->addAnything(_('Classification'), $str_input);
