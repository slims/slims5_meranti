...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
    ...
    if (empty($labelDesc) OR empty($labelName)) {
        ...
        if (isset($_POST['updateRecordID'])) {
            ...
            if ($update) {
                utility::jsAlert(lang_mod_masterfile_label_alert_update_ok);
                echo '<script type="text/javascript">parent.setContent(\'mainContent\', parent.getPreviousAJAXurl(), \'post\');</script>';
            } else { utility::jsAlert(lang_mod_masterfile_label_alert_update_fail."\nDEBUG : ".$sql_op->error); }
            exit();
        } else {
            /* INSERT RECORD MODE */
            // insert the data
            if ($sql_op->insert('mst_label', $data)) {
                utility::jsAlert(lang_mod_masterfile_label_alert_new_add_ok);
                echo '<script type="text/javascript">parent.setContent(\'mainContent\', \''.$_SERVER['PHP_SELF'].'\', \'post\');</script>';
            } else { utility::jsAlert(lang_mod_masterfile_label_alert_add_fail."\nDEBUG : ".$sql_op->error); }
            exit();
        }
    }
    exit();
    ...
    // error alerting
    if ($error_num == 0) {
        utility::jsAlert(lang_mod_masterfile_label_alert_all_delete_ok);
        echo '<script type="text/javascript">parent.setContent(\'mainContent\', \''.$_SERVER['PHP_SELF'].'?'.$_POST['lastQueryStr'].'\', \'post\');</script>';
    } else {
        utility::jsAlert(lang_mod_masterfile_label_alert_all_delete_fail);
        echo '<script type="text/javascript">parent.setContent(\'mainContent\', \''.$_SERVER['PHP_SELF'].'?'.$_POST['lastQueryStr'].'\', \'post\');</script>';
    }
    exit();
}
...
<div class="menuBoxInner masterFileIcon">
    <?php echo strtoupper(lang_mod_masterfile_label); ?> - <a href="#" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>master_file/label.php?action=detail', 'get');" class="headerText2"><?php echo lang_mod_masterfile_label_new_add; ?></a>
    &nbsp; <a href="#" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>master_file/label.php', 'get');" class="headerText2"><?php echo lang_mod_masterfile_label_list; ?></a>
    <hr />
    ...
    <input type="button" id="doSearch" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>master_file/label.php?' + $('search').serialize(), 'post')" value="<?php echo lang_sys_common_form_search; ?>" class="button" />
    </form>
</div>
</fieldset>
<?php
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    if ($can_read AND $can_write) {
        $datagrid->setSQLColumn('lb.label_id',
            'lb.label_desc AS \''.lang_mod_masterfile_label_form_field_label_desc.'\'',
            'lb.label_name AS \''.lang_mod_masterfile_label_form_field_label_name.'\'',
            'lb.last_update AS \''.lang_mod_masterfile_label_common_last_update.'\'');
    } else {
        $datagrid->setSQLColumn('lb.label_desc AS \''.lang_mod_masterfile_label_form_field_label_desc.'\'',
            'lb.label_name AS \''.lang_mod_masterfile_label_form_field_label_name.'\'',
            'lb.last_update AS \''.lang_mod_masterfile_label_common_last_update.'\'');
    }
