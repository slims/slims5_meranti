...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
    ...
    // check form validity
    if (empty($frequency)) {
        utility::jsAlert(lang_mod_masterfile_frequency_alert_name_noempty);
        exit();
    } else {
        ...
        if (isset($_POST['updateRecordID'])) {
            ...
            if ($update) {
                utility::jsAlert(lang_mod_masterfile_frequency_alert_update_ok);
                echo '<script type="text/javascript">parent.setContent(\'mainContent\', parent.getPreviousAJAXurl(), \'post\');</script>';
            } else { utility::jsAlert(lang_mod_masterfile_frequency_alert_update_fail."\nDEBUG : ".$sql_op->error); }
            exit();
        } else {
            /* INSERT RECORD MODE */
            // insert the data
            if ($sql_op->insert('mst_frequency', $data)) {
                utility::jsAlert(lang_mod_masterfile_frequency_alert_new_add_ok);
                echo '<script type="text/javascript">parent.setContent(\'mainContent\', \''.$_SERVER['PHP_SELF'].'\', \'post\');</script>';
            } else { utility::jsAlert(lang_mod_masterfile_frequency_alert_add_fail."\nDEBUG : ".$sql_op->error); }
            exit();
        }
    }
    exit();
...
<div class="menuBoxInner masterFileIcon">
    <?php echo strtoupper(lang_mod_masterfile_frequency); ?> - <a href="#" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>master_file/frequency.php?action=detail', 'get');" class="headerText2"><?php echo lang_mod_masterfile_frequency_new_add; ?></a>
    &nbsp; <a href="#" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>master_file/frequency.php', 'get');" class="headerText2"><?php echo lang_mod_masterfile_frequency_list; ?></a>
    <hr />
    ...
    <input type="button" id="doSearch" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>master_file/frequency.php?' + $('search').serialize(), 'post')" value="<?php echo lang_sys_common_form_search; ?>" class="button" />
    </form>
</div>
</fieldset>
<?php
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    if ($can_read AND $can_write) {
        $datagrid->setSQLColumn('f.frequency_id',
            'f.frequency AS \''.lang_mod_masterfile_frequency_form_field_frequency_name.'\'',
            'l.language_name AS \''.lang_mod_masterfile_lang_form_field_name.'\'',
            'f.time_increment AS \''.lang_mod_masterfile_frequency_form_field_frequency_time_increment.'\'',
            'f.time_unit AS \''.lang_mod_masterfile_frequency_form_field_frequency_unit.'\'',
            'f.last_update AS \''.lang_mod_masterfile_frequency_common_last_update.'\'');
    } else {
        $datagrid->setSQLColumn('f.frequency AS \''.lang_mod_masterfile_frequency_form_field_frequency_name.'\'',
            'l.language_name AS \''.lang_mod_masterfile_lang_form_field_name.'\'',
            'f.time_increment AS \''.lang_mod_masterfile_frequency_form_field_frequency_time_increment.'\'',
            'f.time_unit AS \''.lang_mod_masterfile_frequency_form_field_frequency_unit.'\'',
            'f.last_update AS \''.lang_mod_masterfile_frequency_common_last_update.'\'');
