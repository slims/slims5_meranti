...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
    ...
    if (empty($gmdCode) OR empty($gmdName)) {
        utility::jsAlert(_('GMD Code And Name can\'t be empty'));
        exit();
    } else {
        ...
        if (isset($_POST['updateRecordID'])) {
            ...
            if ($update) {
                utility::jsAlert(_('GMD Data Successfully Updated'));
                echo '<script type="text/javascript">parent.setContent(\'mainContent\', parent.getPreviousAJAXurl(), \'post\');</script>';
            } else { utility::jsAlert(_('GMD Data FAILED to Updated. Please Contact System Administrator')."\nDEBUG : ".$sql_op->error); }
            exit();
        } else {
            /* INSERT RECORD MODE */
            // insert the data
            if ($sql_op->insert('mst_gmd', $data)) {
                utility::jsAlert(_('New GMD Data Successfully Saved'));
                echo '<script type="text/javascript">parent.setContent(\'mainContent\', \''.$_SERVER['PHP_SELF'].'\', \'post\');</script>';
            } else { utility::jsAlert(_('GMD Data FAILED to Save. Please Contact System Administrator')."\nDEBUG : ".$sql_op->error); }
            exit();
        }
    }
    exit();
...
<div class="menuBoxInner masterFileIcon">
    <?php echo strtoupper(_('GMD')); ?> - <a href="#" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>master_file/index.php?action=detail', 'get');" class="headerText2"><?php echo _('Add New GMD'); ?></a>
    &nbsp; <a href="#" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>master_file/index.php', 'get');" class="headerText2"><?php echo _('GMD List'); ?></a>
    <hr />
    ...
    <input type="button" id="doSearch" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>master_file/index.php?' + $('search').serialize(), 'post')" value="<?php echo _('Search'); ?>" class="button" />
    </form>
</div>
</fieldset>
<?php
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    /* Form Element(s) */
    // gmd code
    $form->addTextField('text', 'gmdCode', _('GMD Code').'*', $rec_d['gmd_code'], 'style="width: 20%;" maxlength="3"');
    // gmd name
    $form->addTextField('text', 'gmdName', _('GMD Name').'*', $rec_d['gmd_name'], 'style="width: 60%;"');

    // edit mode messagge
    if ($form->edit_mode) {
        echo '<div class="infoBox">'._('You are going to edit gmd data').' : <b>'.$rec_d['gmd_name'].'</b>  <br />'._('Last Update').$rec_d['last_update'].'</div>'; //mfc
    }
    ...
    if ($can_read AND $can_write) {
        $datagrid->setSQLColumn('g.gmd_id',
            'g.gmd_code AS \''._('GMD Code').'\'',
            'g.gmd_name AS \''._('GMD Name').'\'',
            'g.last_update AS \''._('Last Update').'\'');
    } else {
        $datagrid->setSQLColumn('g.gmd_code AS \''._('GMD Code').'\'',
            'g.gmd_name AS \''._('GMD Name').'\'',
            'g.last_update AS \''._('Last Update').'\'');
    }
