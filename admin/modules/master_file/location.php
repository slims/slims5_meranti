...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
    ...
    if (empty($locationName) OR empty($locationID)) {
        utility::jsAlert(lang_mod_masterfile_location_alert_name_noempty);
        exit();
    } else {
        ...
        if (isset($_POST['updateRecordID'])) {
            ...
            if ($update) {
                utility::jsAlert(lang_mod_masterfile_location_alert_update_ok);
            ...
            } else { utility::jsAlert(lang_mod_masterfile_location_alert_update_fail."\nDEBUG : ".$sql_op->error); }
            exit();
        } else {
            /* INSERT RECORD MODE */
            // insert the data
            ...
            if ($insert) {
                utility::jsAlert(lang_mod_masterfile_location_alert_new_add_ok);
                echo '<script type="text/javascript">parent.setContent(\'mainContent\', \''.$_SERVER['PHP_SELF'].'\', \'post\');</script>';
            } else { utility::jsAlert(lang_mod_masterfile_location_alert_add_fail."\n".$sql_op->error); }
            exit();
        }
    }
    exit();
    ...
    if ($still_have_item) {
        ...
        utility::jsAlert(lang_mod_masterfile_location_alert_not_delete." : \n".$undeleted_locations);
        exit();
    }
    // error alerting
    if ($error_num == 0) {
        utility::jsAlert(lang_mod_masterfile_location_alert_all_delete_ok);
        echo '<script type="text/javascript">parent.setContent(\'mainContent\', \''.$_SERVER['PHP_SELF'].'?'.$_POST['lastQueryStr'].'\', \'post\');</script>';
    } else {
        utility::jsAlert(lang_mod_masterfile_location_alert_all_delete_fail);
        echo '<script type="text/javascript">parent.setContent(\'mainContent\', \''.$_SERVER['PHP_SELF'].'?'.$_POST['lastQueryStr'].'\', \'post\');</script>';
    }
    exit();
}
...
<div class="menuBoxInner masterFileIcon">
    <?php echo strtoupper(lang_mod_masterfile_location); ?> - <a href="#" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>master_file/location.php?action=detail', 'get');" class="headerText2"><?php echo lang_mod_masterfile_location_new_add; ?></a>
    &nbsp; <a href="#" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>master_file/location.php', 'get');" class="headerText2"><?php echo lang_mod_masterfile_location_list; ?></a>
    <hr />
    ...
    <input type="button" id="doSearch" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>master_file/location.php?' + $('search').serialize(), 'post')" value="<?php echo lang_sys_common_form_search; ?>" class="button" />
    </form>
</div>
</fieldset>
<?php
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    /* Form Element(s) */
    // location code
    $form->addTextField('text', 'locationID', lang_mod_masterfile_location_form_field_code.'*', $rec_d['location_id'], 'style="width: 20%;" maxlength="3"');
    // location name
    $form->addTextField('text', 'locationName', lang_mod_masterfile_location_form_field_name.'*', $rec_d['location_name'], 'style="width: 60%;"');

    // edit mode messagge
    if ($form->edit_mode) {
        echo '<div class="infoBox">'.lang_mod_masterfile_location_common_edit_info.'<b>'.$rec_d['location_name'].'</b> <br />'.lang_mod_masterfile_location_common_last_update.$rec_d['last_update'].'</div>';
    }
    ...
    if ($can_read AND $can_write) {
        $datagrid->setSQLColumn('l.location_id',
            'l.location_id AS \''.lang_mod_masterfile_location_form_field_code.'\'',
            'l.location_name AS \''.lang_mod_masterfile_location_form_field_name.'\'',
            'l.last_update AS \''.lang_mod_masterfile_location_common_last_update.'\'');
    } else {
        $datagrid->setSQLColumn('l.location_id AS \''.lang_mod_masterfile_location_form_field_code.'\'',
            'l.location_name AS \''.lang_mod_masterfile_location_form_field_name.'\'',
            'l.last_update AS \''.lang_mod_masterfile_location_common_last_update.'\'');
    }
