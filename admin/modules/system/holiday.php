    ...
    if (empty($holDesc)) {
        ...
        if (isset($_POST['updateRecordID'])) {
            ...
            if ($sql_op->update('holiday', $data, 'holiday_id='.$updateRecordID)) {
                utility::jsAlert(_('Holiday Data Successfully updated'));
                ...
                utility::jsAlert(_('Holiday FAILED to update. Please Contact System Administrator')."\n".$sql_op->error);
            }
        } else {
            /* INSERT RECORD MODE */
            // insert the data
            if ($sql_op->insert('holiday', $data)) {
                utility::jsAlert(_('New Holiday Successfully Saved'));
                // update holiday_dayname session
                $_SESSION['holiday_date'][$data['holiday_date']] = $data['holiday_date'];
                // date range insert
                ...
                utility::jsAlert(_('Holiday FAILED to Save. Please Contact System Administrator')."\n".$sql_op->error);
            }
        }
    }
    exit();
...
<div class="menuBoxInner calendarIcon">
    <?php echo strtoupper(_('Holiday Setting')); ?>
    <hr />
    <a href="#" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>system/holiday.php', 'post');" class="headerText2"><?php echo _('Set holiday'); ?></a>
    &nbsp; <a href="#" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>system/holiday.php?mode=special', 'get');" class="headerText2"><?php echo _('Special holiday'); ?></a>
    &nbsp; <a href="#" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>system/holiday.php?mode=special&action=detail', 'get');" class="headerText2"><?php echo _('Add Special holiday'); ?></a>
</div>
</fieldset>
<?php
if (isset($_GET['mode'])) {
    if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
        ...
        /* Form Element(s) */
        // holiday date start
        $form->addDateField('holDate', _('Holiday Date Start'), $rec_d['holiday_date']);
        // holiday date end
        if (!$form->edit_mode) {
            $form->addDateField('holDateEnd', _('Holiday Date End'), $rec_d['holiday_date']);
        }
        // holiday description
        $form->addTextField('text', 'holDesc', _('Holiday Description').'*', $rec_d['description'], 'style="width: 100%;"');

        // edit mode messagge
        if ($form->edit_mode) {
            echo '<div class="infoBox">'._('You are going to edit holiday data').' : <b>'.$rec_d['description'].'</b></div>'; //mfc
        }
        ...
        if ($can_read AND $can_write) {
            $datagrid->setSQLColumn('holiday_id',
                "holiday_dayname AS '"._('Day name')."'",
                "holiday_date AS '"._('Holiday Date Start')."'",
                "description AS '"._('Holiday Description')."'");
        } else {
            $datagrid->setSQLColumn("holiday_dayname AS '"._('Day name')."'",
                "holiday_date AS '"._('Holiday Date Start')."'",
                "description AS '"._('Holiday Description')."'");
        }
    ...
    if (isset($_POST['dayname'])) {
        ...
        if (count($_POST['dayname']) > 6) {
            echo '<div class="errorBox">'._('Maximum 6 day can be set as holiday!').'</div>';
        } else {
            ...
            if ($_POST['dayname']) {
                ...
                // information box
                echo '<div class="infoBox">'._('Holiday settings saved').'</div>';
            }
        }
    }
    ...
    // dayname list
    $table->appendTableRow(array('<input type="checkbox" name="dayname[]" value="mon" '.isChecked('mon').' /> '._('Monday'),
        '<input type="checkbox" name="dayname[]" value="tue" '.isChecked('tue').' /> '._('Tuesday'),
        '<input type="checkbox" name="dayname[]" value="wed" '.isChecked('wed').' /> '._('Wednesday')));

    $table->appendTableRow(array('<input type="checkbox" name="dayname[]" value="thu" '.isChecked('thu').' /> '._('Thursday'),
        '<input type="checkbox" name="dayname[]" value="fri" '.isChecked('fri').' /> '._('Friday'),
        '<input type="checkbox" name="dayname[]" value="sat" '.isChecked('sat').' /> '._('Saturday')));

    $table->appendTableRow(array('<input type="checkbox" name="dayname[]" value="sun" '.isChecked('sun').' /> '._('Sunday')));
    // set cell attribute
    $table->setCellAttr(3, 0, 'colspan="3"');

    // submit button
    $table->appendTableRow(array('<input type="button" name="saveDaynameData" value="'._('Save Settings').'" onclick="setContent(\'mainContent\', \''.$_SERVER['PHP_SELF'].'\', \'post\', $(\'holidayForm\').serialize())" />'));
    // set cell attribute
