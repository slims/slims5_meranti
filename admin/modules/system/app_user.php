...
if (isset($_POST['saveData'])) {
    ...
    if (empty($userName) OR empty($realName)) {
        utility::jsAlert(_('User Name or Real Name can\'t be empty'));
        exit();
    } else if (($userName == 'admin' OR $realName == 'Administrator') AND $_SESSION['uid'] != 1) {
        utility::jsAlert(_('Login username or Real Name is probihited!'));
        exit();
    } else if (($passwd1 AND $passwd2) AND ($passwd1 !== $passwd2)) {
        utility::jsAlert(_('Password confirmation does not match. See if your Caps Lock key is on!'));
        exit();
    } else {
        ...
        if (isset($_POST['updateRecordID'])) {
            ...
            if ($update) {
                ...
                utility::jsAlert(_('User Data Successfully Updated'));
                echo '<script type="text/javascript">parent.setContent(\'mainContent\', parent.getPreviousAJAXurl(), \'post\');</script>';
            } else { utility::jsAlert(_('User Data FAILED to Updated. Please Contact System Administrator')."\nDEBUG : ".$sql_op->error); }
            exit();
        } else {
            /* INSERT RECORD MODE */
            // insert the data
            if ($sql_op->insert('user', $data)) {
                ...
                utility::jsAlert(_('New User Data Successfully Saved'));
                echo '<script type="text/javascript">parent.setContent(\'mainContent\', \''.$_SERVER['PHP_SELF'].'\', \'post\');</script>';
            } else { utility::jsAlert(_('User Data FAILED to Save. Please Contact System Administrator')."\n".$sql_op->error); }
            exit();
        }
    }
    exit();
...
<div class="menuBoxInner userIcon">
    <?php echo strtoupper(_('System Users')); ?> - <a href="#" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>system/app_user.php?action=detail', 'get');" class="headerText2"><?php echo _('Add New User'); ?></a>
    &nbsp; <a href="#" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>/system/app_user.php', 'get');" class="headerText2"><?php echo _('User List'); ?></a>
    <hr />
    ...
    <input type="button" id="doSearch" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>system/app_user.php?' + $('search').serialize(), 'post')" value="<?php echo _('Search'); ?>" class="button" />
</form>
</div>
</fieldset>
<?php
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    /* Form Element(s) */
    // user name
    $form->addTextField('text', 'userName', _('Login Username').'*', $rec_d['username'], 'style="width: 50%;"');
    // user real name
    $form->addTextField('text', 'realName', _('Real Name').'*', $rec_d['realname'], 'style="width: 50%;"');
    // user group
    ...
    if (!$changecurrent AND $can_read AND $can_write) {
        ...
        $form->addCheckBox('groups', _('Group(s)'), $group_options, unserialize($rec_d['groups']));
    }
    // user password
    $form->addTextField('password', 'passwd1', _('New Password').'*', '', 'style="width: 50%;"');
    // user password confirm
    $form->addTextField('password', 'passwd2', _('Confirm New Password').'*', '', 'style="width: 50%;"');

    // edit mode messagge
    if ($form->edit_mode) {
        echo '<div class="infoBox">'._('You are going to edit user profile'),' : <b>'.$rec_d['realname'].'</b> <br />'._('Last Update').$rec_d['last_update'].' //mfc
            <br />'._('Leave Password field blank if you don\'t want to change the password').'</div>';
    }
    ...
    if ($can_read AND $can_write) {
        $datagrid->setSQLColumn('u.user_id',
            'u.realname AS \''._('Real Name').'\'',
            'u.username AS \''._('Login Username').'\'',
            'u.last_login AS \''._('Last Login').'\'',
            'u.last_update AS \''._('Last Update').'\'');
    } else {
        $datagrid->setSQLColumn('u.realname AS \''._('Real Name').'\'',
            'u.username AS \''._('Real Name').'\'',
            'u.last_login AS \''._('Last Login').'\'',
            'u.last_update AS \''._('Last Update').'\'');
    }
