...
if (isset($_POST['saveData'])) {
    ...
    if (empty($moduleName) OR empty($modulePath)) {
        utility::jsAlert(_('Module name and path can\'t be empty'));
        exit();
    } else {
        ...
        if (isset($_POST['updateRecordID'])) {
            ...
            if ($update) {
                ...
                utility::jsAlert(_('Module Data Successfully Updated'));
                echo '<script type="text/javascript">parent.setContent(\'mainContent\', parent.getPreviousAJAXurl(), \'post\');</script>';
            } else { utility::jsAlert(_('Module Data FAILED to Updated. Please Contact System Administrator')."\nDEBUG : ".$sql_op->error); }
            exit();
        } else {
            /* INSERT RECORD MODE */
            // insert the data
            if ($sql_op->insert('mst_module', $data)) {
                ...
                utility::jsAlert(_('New Module Data Successfully Saved'));
                echo '<script type="text/javascript">parent.setContent(\'mainContent\', \''.$_SERVER['PHP_SELF'].'\', \'post\');</script>';
            } else { utility::jsAlert(_('Module Data FAILED to Save. Please Contact System Administrator')."\n".$sql_op->error); }
            exit();
        }
    }
    exit();
...
<div class="menuBoxInner moduleIcon">
    <?php echo strtoupper(_('Modules')); ?> - <a href="#" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>system/module.php?action=detail', 'get');" class="headerText2"><?php echo _('Add New Modules'); ?></a>
    &nbsp; <a href="#" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>system/module.php', 'get');" class="headerText2"><?php echo _('Modules List'); ?></a>
    <hr />
    ...
    <input type="button" id="doSearch" onclick="setContent('mainContent', '<?php echo MODULES_WEB_ROOT_DIR; ?>system/module.php?' + $('search').serialize(), 'post')" value="<?php echo _('Search'); ?>" class="button" />
    </form>
</div>
</fieldset>
<?php
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    /* Form Element(s) */
    // module
    $form->addTextField('text', 'moduleName', _('Module Name').'*', $rec_d['module_name'], 'style="width: 50%;"');
    // module path
    $form->addTextField('text', 'modulePath', _('Module Path').'*', $rec_d['module_path'], 'style="width: 100%;"');
    // module desc
    $form->addTextField('text', 'moduleDesc', _('Module Description'), $rec_d['module_desc'], 'style="width: 100%;"');

    // edit mode messagge
    if ($form->edit_mode) {
        echo '<div class="infoBox">'._('You are going to edit data').' : <b>'.$rec_d['module_name'].'</b></div>'; //mfc
    }
    ...
    if ($can_read AND $can_write) {
        $datagrid->setSQLColumn('mdl.module_id',
            'mdl.module_name AS \''._('Module Name').'\'',
            'mdl.module_desc AS \''._('Module Description').'\'');
    } else {
        $datagrid->setSQLColumn('mdl.module_name AS \''._('Module Name').'\'',
            'mdl.module_desc AS \''._('Module Description').'\'');
    }
