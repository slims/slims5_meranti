...
class serial
{
    ...
    public function viewKardexes()
    {
        ...
        $_table->setHeader(array('&nbsp;', _('Date Expected'),
            _('Date Received'), _('Seq. Number'),
            _('Note')));
        ...
        // button
        $_button_grp = '<div style="padding: 3px; background: #999999;"><input type="submit" class="button" name="saveKardexes" value="'._('Save').'" /></div>';

        // header
        echo '<div style="padding: 5px; background: #CCCCCC;">'._('Kardex Detail for subscription').' <strong>'.$this->serial_period.'</strong></div>';
        if ($_can_read AND $_can_write) {
