...
if ($unauthorized) {
    ...
    $msg .= 'alert(\''._('You are not authorized to view this section').'\');'."\n";
...
if ($unauthorized) {
    ...
    $msg .= _('You are not authorized to view this section');
    $msg .= '</div>'."\n";
    simbio_security::destroySessionCookie($msg, SENAYAN_SESSION_COOKIES_NAME, SENAYAN_WEB_ROOT_DIR, true);
}
...
if ($timeout) {
    $msg = '<div style="padding: 5px; border: 1px dotted #FF0000; color: #FF0000;">';
    $msg .= _('Your Login Session has already timeout!').' <a href="'.SENAYAN_WEB_ROOT_DIR.'index.php?p=login">Re-Login</a>';
    $msg .= '</div>'."\n";
    simbio_security::destroySessionCookie($msg, SENAYAN_SESSION_COOKIES_NAME, SENAYAN_WEB_ROOT_DIR, true);
} else {
